...
The benchmark is levered beta on four assets that all rose 2018–25; the incumbent `trend_long_only` lost to it (0.80 vs 1.13) because its fast per-asset components whipsaw at a 29 bps round trip. Hypothesis: a slow (6- or 12-month) absolute-momentum gate judged per bloc (equal-weight of two correlated assets) changes state a few times a year, keeps the bull exposure, sits in 3.25% collateral through the 2018 and 2022 bear legs, and relative momentum tilts risk to the trending bloc. Mechanism: a bloc's trailing return over `lookback` calendar days (ending `skip` days ago) must exceed 3.25% × lookback/365; the higher-return bloc gets risk share `top_share` (winner sleeve at the benchmark's size, loser at (1−top_share)/top_share of it, a gated-out bloc 0); long-only, equal risk per asset via `vol_target`. Evidence: Antonacci, "Risk Premia Harvesting Through Dual Momentum" (SSRN 2042750) and "Absolute Momentum" (SSRN 2244633); Moskowitz–Ooi–Pedersen 2012 "Time Series Momentum" (JFE); Hurst–Ooi–Pedersen 2017 "A Century of Evidence on Trend-Following Investing" (SSRN 2993026); Jegadeesh–Titman 1993 for the skip month; Liu–Tsyvinski 2021 (RFS) for crypto TSMOM; `tournament.md` for the whipsaw diagnosis.

## 2. Data and causality
Daily closes from `D.load_panel` only (Coinbase spot, Yahoo GC=F/SI=F); no aux series. `align()` closes are resampled to a calendar-daily forward-filled frame so the horizon is identical for crypto (365 bars/yr) and metals (252). Row t uses close(t−skip) and close(t−skip−lookback); weights decided at close t execute at t+1's open. An asset joins its bloc and is tradable only if it printed a native bar within the last 7 days at every point of the window, which drops xrp through its 2021-01-19 → 2023-07-13 Coinbase delisting and until 2024-07-12 (else the relisting jump counts as momentum). `skip` is clipped at 0. Registry-wide causality test green (38 tests OK); truncating real data at 2022-07-01 changes no earlier weight (max |diff| 0.0). Nothing after 2025-07-01 was read.
...
```
Six configurations; lookbacks in calendar days (182 ≈ 6 months, 365 = 12). `dual_momentum_6` ran with `--universe btc,eth,gold,silver,sol,xrp --start 2021-07-01` (shorter sample). Nothing was added after seeing results.
...
`dual_momentum_skip` (fixed_params=true; OOS SR by fold 0.568, 0.393, 2.979, -0.288, 1.804, 0.18): oos_sharpe_concat 1.047, oos_cagr 0.1198, oos_max_drawdown -0.0865, folds positive 6/6, WFE null.
...
net_sharpe 0.651, cagr 0.0845, max_drawdown -0.1246, total_return 0.8379, ann_turnover 2.4 (incumbent 3.76, benchmark 1.68), fees_annual 0.0035, avg_gross_leverage 0.188, n_liquidations 0. By year: 2018 0.0032, 2019 0.0484, 2020 0.0808, 2021 0.2041, 2022 0.0508, 2023 0.0873, 2024 0.1372, 2025 0.0333.
...
Bootstrap Sharpe p5/p50/p95 -0.0418 / 0.6554 / 1.278 (p_sharpe_le_0 0.062); CAGR 0.0262 / 0.0847 / 0.1397; MDD -0.2211 / -0.126 / -0.0702. Perturbation (10): min 0.469 (lookback 136), median 0.641, max 0.788 (target_vol 0.09), collapse_ratio 0.72. Timing null: real 0.651, null mean 0.575, null p95 0.98, percentile_of_real 0.617. Regime: btc_up_years Sharpe 0.835, btc_down_years -0.082; vol calm 1.437, mid 0.376, turbulent 0.281. Sharpe difference vs benchmark: diff -0.289 (p5 -0.797, p95 0.201), **p_a_gt_b 0.148**, corr 0.683.
...
The whipsaw moved rather than vanished: the 6-month gate re-entered late after the 2019 and 2022–23 recoveries (folds 1 and 3 negative) and metals flicker around the 3.25% hurdle (9–15 signal changes per asset-year before the weekly band); a timing-null percentile of 0.617 says the gate barely beats its own exposure shuffled in time. Untested: (a) hysteresis around the hurdle; (b) vol-adjusted relative momentum so metals can win; (c) blending 182 and 365 instead of selecting (WFE 0.195); (d) handing the loser's budget to the winner when only one bloc passes. Each is a new configuration that raises the registry's N for everyone, so none was run.
