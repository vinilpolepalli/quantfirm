...
def implied_sigma_1m(s: float, k: float, mid: float, tau_minutes: float) -> float | None:
    """Invert P = N(ln(S/K)/(σ√τ)) for one-minute log-return vol.

    Unstable near 50¢ or S≈K; returns None when the inversion is not
    identified. Used by the IV-rich-favorite strategy, not for pricing.
    """
    if tau_minutes <= 0 or s <= 0 or k <= 0:
        return None
    if mid <= 0.02 or mid >= 0.98:
        return None
    # binary search the standard normal quantile
    lo, hi = -8.0, 8.0
    for _ in range(40):
        mid_z = (lo + hi) / 2
        if norm_cdf(mid_z) < mid:
            lo = mid_z
        else:
            hi = mid_z
    z = (lo + hi) / 2
    if abs(z) < 1e-6:
        return None
    return abs(math.log(s / k)) / (abs(z) * math.sqrt(tau_minutes))
