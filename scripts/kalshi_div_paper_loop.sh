...
#
# Same supervisor shape as kalshi_poly_paper_loop.sh:
...
LOG="${LOG:-state/kalshi_div_paper_loop.log}"
PIDFILE="${PIDFILE:-state/kalshi_div_paper.pid}"
DECISIONS="state/kalshi_div_paper_decisions.jsonl"
STALE_S="${STALE_S:-300}"
...
log "div paper supervisor start (pid $$): ${SESSION_MIN}min sessions, metals=${METALS} live=0"
...
while true; do
  ...
  log "starting ${SESSION_MIN}min desk_book paper session on ${METALS} (no live)"
  ...
  while kill -0 "$engine_pid" 2>/dev/null; do
    ...
    if [ "$age" -gt "$STALE_S" ]; then
      ...
      # SIGKILL on `timeout` does not reap its python child.
